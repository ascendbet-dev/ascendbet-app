...
// 🔥 FORCE FRESH SESSION (THIS FIXES YOUR ISSUE)
...
const {
  ...
  if (activeSeason) {
    const { data: instance } = await supabase
      ...
      .select(
        "id, current_balance, target_balance, hard_drawdown, starting_balance, settled_bet_count, discipline_score, active_betting_days, is_disqualified, is_qualified, last_bet_at"
      )
      .eq("user_id", user.id)
  ...
  if (season.last_bet_at) {
    const now = new Date();
    const lastBet = new Date(season.last_bet_at);

    const hoursInactive =
      (now.getTime() - lastBet.getTime()) / (1000 * 60 * 60);

    if (hoursInactive >= 24 && hoursInactive < 48) {
      const remaining = Math.round(48 - hoursInactive);

      warning = `⚠️ You haven’t placed a bet in ${Math.floor(
        hoursInactive
      )}hrs. A 2% penalty will be apply to your balance in the next ${remaining}hrs.`;
    }

    if (hoursInactive >= 48) {
      warning = "🔥 2% penalty active due to inactivity.";
    }
  }
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100dvh-120px)] max-w-lg flex-col px-4 py-2">
      ...
      {/* 🔥 WARNING MESSAGE */}
      {warning && (
        <div className="mb-2 rounded-md border border-yellow-500/20 bg-yellow-500/5 px-3 py-2">
